...
describe('test ABLS Widget', () => {
  it('basic render', () => {
    ...
    const Widget = wrapWidget(_Widget, {
      config: { views: [{ id: 'view_1', name: 'View 1', icon: null, layerIds: [], }] },
  ...
  it('click view button', async () => {
    //const viewConfig: ABLSView = { id: 'view_1', name: 'View 1', icon: null, layerIds: [], }
    const Widget = wrapWidget(_Widget, {
      config: { views: [{ id: 'view_1', name: 'View 1', icon: null, layerIds: [] }, { id: 'view_2', name: 'View 2', icon: null, layerIds: ['123','456'] }] },
      useMapWidgetIds: ['mapWidget_1'] as unknown as ImmutableArray<string>,
    })
  ...
  it('not configured', () => {
    ...
    const { queryByText } = render(<Widget widgetId="Widget_1" />)
    expect(queryByText('Please configure this widget in the settings panel.')).toBeTruthy()
  })
})